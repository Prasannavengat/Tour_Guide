        ...
        card.innerHTML = `
            ...
            <div class="place-card-info">
                <div class="place-card-info-row">
                    ...
                    <div class="place-card-info-row">
                        ${place.facilities ? `
                          <span style="margin-right:12px"><i class="fas fa-hospital"></i> ${place.facilities.hospital.length}</span>
                          <span style="margin-right:12px"><i class="fas fa-hotel"></i> ${place.facilities.hotel.length}</span>
                          <span><i class="fas fa-shopping-bag"></i> ${place.facilities.essentials.length}</span>
                        ` : ''}
                    </div>
            </div>
...
async function loadFacilities(type) {
    try {
        ...
        // If site has precomputed facilities, use them as a fallback (avoids upstream rate limits)
        if (currentPlace.facilities && Array.isArray(currentPlace.facilities[type]) && currentPlace.facilities[type].length) {
            allFacilities = currentPlace.facilities[type] || [];
